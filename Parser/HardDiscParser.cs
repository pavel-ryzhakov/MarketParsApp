...
namespace MarketParsApp.Parser
{
    internal class HardDiscParser
    {
        ...
        public HardDiscParser(IDbContextFactory<DataBaseContext> contextFactory)
        {
...

        {
            ...
            //            ctx.Refresh();
                        ...
                        foreach (var product in products)
                        {
                            ...
                            try
                            {
                                ...
                                entity.Capacity = Int32.Parse(Regex.Replace(doc.QuerySelector("td#tdsa3978").FirstChild.TextContent, @"\D+.", "", RegexOptions.ECMAScript));
                                entity.ReadSpeed = doc.QuerySelector("td#tdsa1519").FirstChild.TextContent;
                                entity.WriteSpeed = doc.QuerySelector("td#tdsa655").FirstChild.TextContent; 
                                entity.Format = doc.QuerySelector("td#tdsa3997").FirstChild.TextContent;
                                ...
                                //ctx.Refresh();
                                await context.HardDisks.AddAsync(entity);
                                _count++;
            ...
            AnsiConsole.Markup($"[green on yellow] {_count} [/]");
            AnsiConsole.WriteLine();
