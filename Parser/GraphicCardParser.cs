...
{
    internal class GraphicCardParser
    {
        ...
        public GraphicCardParser(IDbContextFactory<DataBaseContext> contextFactory)
        {
...

        {
            ...
            //            ctx.Refresh();
                        ...
                        foreach (var product in products)
                        {
                            ...
                            try
                            {
                                ...
                                entity.GpuName = doc.QuerySelector("td#tdsa4190 ").FirstChild.TextContent;
                                entity.Power =Int32.Parse(Regex.Replace(doc.QuerySelector("td#tdsa44456").FirstChild.TextContent, @"\D+.", "", RegexOptions.ECMAScript));
                                entity.Vram = doc.QuerySelector("td#tdsa689").FirstChild.TextContent;
                                entity.VramType = doc.QuerySelector("td#tdsa4187").FirstChild.TextContent;
                                entity.Frequency = doc.QuerySelector("td#tdsa4191").FirstChild.TextContent;
                                ...
                                //ctx.Refresh();
                                await context.GraphicCards.AddAsync(entity);
                                _count++;
                                AnsiConsole.WriteLine($"[white on green] Добавлен {_count} [/]");
            ...
            //AnsiConsole.Markup($"[default on grey] найдено единиц товаров [/] за [white on black]{timer.Stop()}");
